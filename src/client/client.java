...
public class client
{
	...
	public static void main(String args[])
	{
		...
		/*
		 * if (args.length > 0) { server = args[0]; } if (args.length > 1) { port = Integer.parseInt(args[1]); } if
		 * (args.length > 2) { System.out.println("Usage: java client [rmihost [rmiport]]"); System.exit(1); }
		 */
		...
		try
		{
			...
			rm = (ResourceManager) registry.lookup("30Middleware");
